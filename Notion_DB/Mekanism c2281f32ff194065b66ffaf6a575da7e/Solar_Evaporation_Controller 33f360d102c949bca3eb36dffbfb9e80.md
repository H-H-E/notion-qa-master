...
Used as a component/block to build the multi-block Thermal Evaporation Plant. The controller for a Thermal Evaporation Plant, acting as the master block of the structure and the heart of the Thermal Evaporation Plant. Only one of these should be placed on a multiblock. This block is the main way that players access the Thermal Evaporation Plant. When building the Thermal Evaporation Plant, the Thermal Evaporation Controller must be placed on the top layer.
